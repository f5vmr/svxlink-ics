...
#	propagationmonitor
	echo -e "Unused logic module: $NOT_LOGIC_MODULE" | sudo tee -a /var/log/install.log 
...
if ! grep -q "^\[$NOT_LOGIC_MODULE\]" "$CONF"; then
    echo -e "Module [$NOT_LOGIC_MODULE] not found in $CONF" | sudo tee -a /var/log/install.log
else echo -e "Module [$NOT_LOGIC_MODULE] found in $CONF, proceeding to remove it." | sudo tee -a /var/log/install.log 	 
fi
...
sed -i "/^\[$NOT_LOGIC_MODULE\]/,/^\[/{
	...
	if grep -q "^\[$NOT_LOGIC_MODULE\]" "$CONF"; then
	echo -e "Failed to remove module [$NOT_LOGIC_MODULE] from $CONF" | sudo tee -a /var/log/install.log 
	fi
	 # clear
